...
const screenForNames = (arr) => {
  ...
  let Arr = [];
  arr.map(char => {
    if(/^(Mr. |Mrs. |Ms. |Dr. )[a-zA-Z]/g.test(char) ){
      Arr.push(char)
    }
  })
  return Arr;
...
const toTitleCase = (arr) => {
  ...
  let TitleCase = arr.map(value => value.charAt(0).toUpperCase() + value.slice(1));
   return TitleCase ;
  };
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let biggerThanLuke = arr.filter((element) => element.mass > 77).map((element) => element.name).join(' - ');
  return biggerThanLuke;
};
