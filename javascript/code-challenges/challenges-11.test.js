...
const count = (target, input) => {
 ...
 let Element = Array.filter(value => value === target);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(value => value.filter(element => ((typeof element) === 'number') && (element %5 === 0)).map(value => Math.pow(2,value)));
};
...
let findMaleAndFemale = (data) => {
  ...
  data.map((value , index) => {
    ...
    else if (value.gender === 'female' || value.gender === 'male'){
      personName += value.name + ' and ';
    }
  })
  return personName;
};
...
let findShortest = (data) => {
  ...
  return data.reduce((number1,number2) => 
  Number(number1.height) < Number(number2.height) ? number1 : number2 ).name;
