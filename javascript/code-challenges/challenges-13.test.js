...
const firstLetters = (arr) => {
  // Solution code here...
  let str = arr.map(element => element.charAt(0));
return str;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let char = /([()-\s])/g;
  return arr.map(element => element.replace(char,'')); 
};
...
const allHappy = (arr) => {
  // Solution code here...
 let happy = arr.length === arr.filter(element => element.includes(`:)`)).length;
 return happy;
};
